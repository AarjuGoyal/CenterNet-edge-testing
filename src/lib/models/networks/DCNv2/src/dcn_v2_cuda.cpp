                         ...
                         const int deformable_group)
{
    ...
    for (int b = 0; b < batch; b++)
    {
        ...
        output_n.copy_(bias_output.transpose(0,1).view_as(output_n)); //ideallly the output of this should be channels_out * height_out * width_out
        ...
        output_n.addmm_(columns_mat, weights_mat, 1.0f, 1.0f); // 
                          ...
                          int deformable_group)
{
    ...
    for (int b = 0; b < batch; b++)
    {
        ...
        columns->copy_(grad_output_mat.mm(weight_mat)); //.matmul(grad_output_mat, weight_mat);
        ...
        auto columns_mat = (columns->view({kw_, nw_})).transpose(0,1);
        auto grad_output_n_mat = grad_output_n.view({kw_, mw_});
        grad_weight->addmm(columns_mat, grad_output_n_mat, 1.0f, 1.0f);
